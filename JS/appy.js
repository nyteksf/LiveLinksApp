  ...
  function salty() {
    let charBank = '0123456789ABCDEFGHIJKLMNO';
    let rand = '';
    for (var i = 0; i < 5; i++ ) {
        rand += charBank[Math.floor(Math.random() * 24)];
    }
  ...
  let keyGen = function(uid) {
    ...
    let part4 = salty(); //AND THEN ENCRYPTED
    let key = ""+part1+part3+part4+part2; //FOR MORE RANDOMNESS

    key = crypt.AES.encrypt(key);
    firebase.database().ref('/users/'+uid).update({ key: true });
  ...
  setTimeout(function() {
    ...
    if(localStorage["lastProvider"] === "password" && localStorage["autoLogin"] === "true") {
      let k;
      let encryptedPW;
      let user;
